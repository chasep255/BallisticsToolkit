...
export class FlagSystem
{
  ...
  createFlagAtPosition(xPosition, zPosition)
  {
    ...
    this.flagMeshes.push({
      ...
      flagMesh: flagMesh,
      position: { x: xPosition, y: flagY, z: zPosition },
  ...
  updateFlags(deltaTime, currentTime, windGenerator)
  {
    ...
    for (let i = 0; i < this.flagMeshes.length; i++)
    {
      ...
      const windVector = windGenerator.getWindAt(pos.x, pos.y, pos.z, currentTime);

      // Wind vector is in mph
      const windX_mph = windVector.x;
      const windY_mph = windVector.y;  
      const windZ_mph = windVector.z;
      ...
      const targetAngleDeg = Math.min(
        this.cfg.flagMinAngle + windSpeedMph * this.cfg.flagDegreesPerMph,
        this.cfg.flagMaxAngle
      );

      // Calculate wind direction - flag points where wind blows TO
      const targetDirection = Math.atan2(windZ_mph, windX_mph);
      ...
      const flapFrequency = this.cfg.flagFlapFrequencyBase + windSpeedMph * this.cfg.flagFlapFrequencyScale;
