...
export class RenderStats
{
  ...
  /**
   * Log current frame draw calls immediately (for per-frame logging)
   * Assumes stats have been reset at frame start, so these are per-frame values
   */
  logFrameDrawCalls()
  {
    ...
    {
      // Since we reset at frame start, calls is the total for this frame
      calls.push(`${name}: ${stats.calls}`);
    }

    console.log(`[RenderStats] Draw calls: ${calls.join(', ')}`);
  }
