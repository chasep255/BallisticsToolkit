...
namespace btk::math
{
  ...
  class PerlinNoise
  {
    ...
    // ------------------------------------------------------------
    // fBm helpers (multi-octave, normalized to ~[-1,1])
    // ------------------------------------------------------------
    inline float fbm1D(float x, int oct, float lac = 2.0f, float gain = 0.5f) const noexcept
    {
      float a = 1.0f, f = 1.0f, sum = 0.0f, asum = 0.0f;
      for(int i = 0; i < oct; ++i)
      {
        sum += a * noise1D(x * f);
        asum += a;
    ...
    inline float fbm2D(float x, float y, int oct, float lac = 2.0f, float gain = 0.5f) const noexcept
    {
      float a = 1.0f, f = 1.0f, sum = 0.0f, asum = 0.0f;
      for(int i = 0; i < oct; ++i)
      {
    ...
    inline float fbm3D(float x, float y, float z, int oct, float lac = 2.0f, float gain = 0.5f) const noexcept
    {
      float a = 1.0f, f = 1.0f, sum = 0.0f, asum = 0.0f;
      for(int i = 0; i < oct; ++i)
      {
    ...
    inline float fbm4D(float x, float y, float z, float w, int oct, float lac = 2.0f, float gain = 0.5f) const noexcept
    {
      float a = 1.0f, f = 1.0f, sum = 0.0f, asum = 0.0f;
      for(int i = 0; i < oct; ++i)
      {
        sum += a * noise4D(x * f, y * f, z * f, w * f);
        asum += a;
        f *= lac;
        a *= gain;
      }
    ...
    static inline float to01(float v) noexcept { return 0.5f * (v + 1.0f); }
