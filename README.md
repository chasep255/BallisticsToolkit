...
- **Physics-Based Ballistics** - Modified point-mass trajectory simulation with G1/G7 drag models, wind presets, and Litz-based spin drift + crosswind jump. Muzzle velocity variation (MV σ) and rifle accuracy (MOA) model shot-to-shot spreads.
...
7. **Remote Play (Beta)** - Check **Host Remote Play** before starting to stream the match to another player. It generates one invite link, send it to them and they open it to start playing (no codes to copy back). The same link reconnects them if they drop. The viewer mirrors the host (live view + audio, scorecard, controls); in Pair Fire the host plays Player 1 and the remote player Player 2, with controls limited to whoever's turn it is. Best on ordinary home/office networks, may fail over cellular or VPN
