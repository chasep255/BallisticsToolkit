...
int main()
{
  ...
  for(int x = 0; x <= 1000; x += 200)
  {
    for(int t = 0; t <= 10; t += 5)
    {
      ...
      std::cout << std::setw(12) << x << " | " << std::setw(8) << t << " | " << std::setw(15) << std::fixed << std::setprecision(3) << wind_vec.y << " | " << std::setw(14) << wind_vec.x << "\n";
    }
  }
  ...
  for(const auto& preset : presets)
  {
    ...
    std::cout << "  Sample at (500m, 5s): cross=" << std::fixed << std::setprecision(3) << sample.y << " m/s, head=" << sample.x << " m/s\n";
  }
