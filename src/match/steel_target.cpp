...
namespace btk::match
{
  ...
  SteelTarget::SteelTarget(float width, float height, float thickness, bool is_oval)
    : width_(width), height_(height), thickness_(thickness), is_oval_(is_oval), position_(0.0f, 0.0f, 0.0f), normal_(1.0f, 0.0f, 0.0f),
      ...
      velocity_ms_(0.0f, 0.0f, 0.0f), angular_velocity_(0.0f, 0.0f, 0.0f), mass_kg_(0.0f), inertia_tensor_(0.0f, 0.0f, 0.0f), linear_damping_(DEFAULT_LINEAR_DAMPING),
      angular_damping_(DEFAULT_ANGULAR_DAMPING), segments_per_circle_(32), texture_width_(512), texture_height_(512)
  {
  ...
  void SteelTarget::updateDisplay()
  {
    ...
    if(is_oval_)
    {
      ...
      for(int i = 0; i < segments_per_circle_; ++i)
      {
        ...
        // BTK: X=downrange, Y=crossrange, Z=up (meters)
